...
With [go-orbit-db](https://github.com/berty/go-orbit-db)'s decline in activity
and the issues arising from trying to keep up with ~~IPFS~~
[Kubo](https://github.com/ipfs/kubo), _Superhighway84_ is pretty much _dead in
the water_. While it is perfectly usable in its current state, it depends on
obsolete versions of ~~IPFS~~ Kubo and its dozens of dependencies.

If you are looking for a _spiritual successor_ to _Superhighway84_, check out
[_Neon Modem Overdrive_](http://github.com/mrusme/neonmodem). _NMO_ is a TUI
client for _Discourse_, _Lemmy_, _Lobsters_ and _Hacker News_, featuring similar
aesthetics. However, _NMO_ does not (yet?) offer an implementation for a
decentralized community like _Superhighway84_.
...
Download the
[kubo 0.16 release](https://github.com/ipfs/kubo/releases/tag/v0.16.0) and
unpack it:
...
Download the
[latest release](https://github.com/mrusme/superhighway84/releases/latest) and
unpack it:
...
Second, if your hardware shouldn't be a beefy computer but instead one of those
flimsy MacBooks, older hardware, a Raspberry or a low-memory VPS it is advisable
to set the previously created IPFS repository to the `lowpower` profile.
...
- `Newsgroup:`\
  The newsgroup under which the article will be submitted, this can either be an
  existing group or a new group. Please try to follow the convention when
  creating new groups. The newsgroup must only contain of printable ASCII
  characters.

The SEPARATOR contains of 6 equal signs and 5 spaces, alternating each other,
...
- Superhighway84 is bound to the version of IPFS that Berty decides to support
  ...
  now, so Superhighway84 is basically stuck with it. If you happen to know your
  way around IPFS and maybe even go-orbit-db, and would like to support this
